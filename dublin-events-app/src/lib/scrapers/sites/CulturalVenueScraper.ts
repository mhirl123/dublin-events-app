import { BaseScraper, ScrapedEvent } from '../BaseScraper'
import * as cheerio from 'cheerio'

/**
 * Cultural venue scraper for arts, classical music, and performance spaces
 * Examples: National Concert Hall, RDS, Helix, etc.
 */
export class CulturalVenueScraper extends BaseScraper {
  protected venueName: string
  protected venueUrl: string
  protected eventSelector: string
  protected titleSelector: string
  protected dateSelector: string
  protected priceSelector: string
  protected ticketUrlSelector: string
  protected genre: string

  constructor(
    venueName: string,
    venueUrl: string,
    genre: string = 'Art',
    eventSelector: string = '.event',
    titleSelector: string = '.event-title',
    dateSelector: string = '.event-date',
    priceSelector: string = '.event-price',
    ticketUrlSelector: string = 'a.tickets'
  ) {
    super(venueName, venueUrl, 'cheerio')
    this.venueName = venueName
    this.venueUrl = venueUrl
    this.genre = genre
    this.eventSelector = eventSelector
    this.titleSelector = titleSelector
    this.dateSelector = dateSelector
    this.priceSelector = priceSelector
    this.ticketUrlSelector = ticketUrlSelector
  }

  async scrape(): Promise<ScrapedEvent[]> {
    console.log(`[${this.venueName}] Starting scrape from ${this.venueUrl}...`)
    const events: ScrapedEvent[] = []

    try {
      const response = await fetch(this.venueUrl, {
        headers: {
          'User-Agent':
            'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
        },
      })

      if (!response.ok) {
        throw new Error(`HTTP ${response.status}`)
      }

      const html = await response.text()
      const $ = cheerio.load(html)

      // Parse events from the page
      $(this.eventSelector).each((index, element) => {
        try {
          const $event = $(element)

          const title = $event.find(this.titleSelector).text().trim()
          const dateStr = $event.find(this.dateSelector).attr('data-date') ||
            $event.find(this.dateSelector).text().trim() || null
          const priceStr = $event.find(this.priceSelector).text().trim()
          const ticketLink =
            $event.find(this.ticketUrlSelector).attr('href') || null

          if (!title || !dateStr) return // Skip if missing critical data

          // Parse date
          let eventDate: Date
          try {
            eventDate = this.parseDate(dateStr)
          } catch {
            console.warn(`[${this.venueName}] Could not parse date: ${dateStr}`)
            return
          }

          events.push({
            title,
            dateStart: eventDate,
            venueName: this.venueName,
            genre: this.genre,
            ticketPriceMin: this.parsePrice(priceStr),
            ticketUrl: ticketLink
              ? ticketLink.startsWith('http')
                ? ticketLink
                : `${this.venueUrl}${ticketLink}`
              : undefined,
            sourceUrl: this.venueUrl,
          })
        } catch (error) {
          console.error(`[${this.venueName}] Error parsing event:`, error)
        }
      })

      console.log(`[${this.venueName}] Found ${events.length} events`)
    } catch (error) {
      console.error(`[${this.venueName}] Scrape failed:`, error)
    }

    // Save to database
    if (events.length > 0) {
      await this.saveEvents(events)
    }

    await this.cleanup()
    return events
  }

  protected parseDate(dateStr: string): Date {
    const date = new Date(dateStr)
    if (isNaN(date.getTime())) {
      throw new Error(`Invalid date: ${dateStr}`)
    }
    return date
  }

  protected parsePrice(priceStr: string): number | undefined {
    if (!priceStr) return undefined
    const match = priceStr.match(/€?([\d.]+)/)
    return match ? parseFloat(match[1]) : undefined
  }
}

// Pre-configured scrapers for Dublin cultural venues
export class NationalConcertHallScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'National Concert Hall',
      'https://www.nch.ie',
      'Music',
      '.event-item',
      '.event-title',
      '.event-date',
      '.price',
      'a.book-ticket'
    )
  }
}

export class RDSScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'RDS',
      'https://www.rds.ie',
      'Festival',
      '.show',
      '.show-name',
      '[data-date]',
      '.cost',
      'a.tickets'
    )
  }
}

export class TheHelixDCUScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'The Helix DCU',
      'https://www.thehelix.ie',
      'Music',
      '.event-card',
      '.event-name',
      '.event-when',
      '.ticket-price',
      'a.book-now'
    )
  }
}

export class BordGaisScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'Bord Gáis Energy Theatre',
      'https://www.bordgaisenergytheatre.ie',
      'Theater',
      '.production',
      '.prod-title',
      '.prod-dates',
      '.ticket-price',
      'a.book-seats'
    )
  }
}

export class SampsonPlaceScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'Sampson Place',
      'https://www.sampsondublin.ie',
      'Art',
      '.exhibition',
      '.title',
      '.dates',
      '.admission',
      'a.learn-more'
    )
  }
}

export class DublinTheatreFestivalScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'Dublin Theatre Festival',
      'https://www.dublintheatrefestival.ie',
      'Theater',
      '.show-item',
      '.show-title',
      '.show-dates',
      '.price',
      'a.book-tickets'
    )
  }
}

export class ConventionCentreScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'Convention Centre Dublin',
      'https://www.theccd.ie',
      'Conference',
      '.event-listing',
      '.event-name',
      '.event-date',
      '.price',
      'a.register'
    )
  }
}

export class GaietyTheatreScraper extends CulturalVenueScraper {
  constructor() {
    super(
      'Gaiety Theatre',
      'https://gaietytheatre.ie',
      'Theater',
      '.performance',
      'h3',
      '.perf-date',
      '.perf-price',
      'a[href*="tickets"]'
    )
  }
}
